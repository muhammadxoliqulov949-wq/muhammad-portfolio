...
| Bo'limlar | Hero, ishlar (featured + ro'yxat), xizmatlar, tajriba (timeline), toolbox (skill matritsasi), mijozlar fikri, aloqa, footer, mobil sticky CTA |
| Case study | `/projects` arxivi + `/projects/[id]` — SSG, muammo → yechim → natija, galleriya, oldingi/keyingi loyiha, breadcrumbs, canonical |
| Admin | Profil + 5 kollekisiya (CRUD, drag'siz ↑↓ tartib, `published` toggle, qidiruv, skeleton, toast + undo, saqlanmagan o'zgarishdan ogohlantirish), xabarlar (filtr, optimistic read, reply), JWT login |
...
| Tekshiruv | `npm run lint`, `npm run typecheck`, `npm run build`, **`npm run smoke`** — 48 ta real HTTP tekshiruv |
...
8 jadval: `admins` (+ `sessions_revoked_at`), `profile`, `projects` (case study ustunlari: `problem`, `approach`, `outcome`, `impact`, `gallery`), `services`, `skills` (+ `years`, `context`, `category`), `experience` (+ `current`), `testimonials` (+ `rating`, `avatar_initials`), `messages` (+ `read`).
